...
export default function AdminUsersPage() {
  ...
  const downloadDonorsReport = () => {
    ...
    const totalPaid = filteredUsers.reduce((sum, u) => sum + u.stats.totalContributed, 0);
    const totalUnpaid = filteredUsers.reduce((sum, u) => sum + u.stats.pendingAmount, 0);
    const totalContributors = totalPaid + totalUnpaid;
    const totalTransactions = filteredUsers.reduce((sum, u) => sum + u.stats.totalTransactions, 0);
    
    doc.setFontSize(9);
    doc.setTextColor(50, 50, 50);
    doc.text(`Total Contributors: ${formatCurrency(totalContributors)}`, 14, finalY + 22);
    doc.text(`Paid Amount: ${formatCurrency(totalPaid)}`, 14, finalY + 28);
    doc.text(`Unpaid Amount: ${formatCurrency(totalUnpaid)}`, 14, finalY + 34);
    doc.text(`Total Transactions: ${totalTransactions}`, 14, finalY + 40);
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
              <div className="bg-emerald-50 p-3 sm:p-4 rounded-lg border border-emerald-200">
                <p className="text-xs sm:text-sm text-emerald-700 font-medium">Total Contributors</p>
                <p className="text-xl sm:text-2xl font-bold text-emerald-900">
                  {formatCurrency(filteredUsers.reduce((sum, u) => sum + u.stats.totalContributed + u.stats.pendingAmount, 0))}
                </p>
                <p className="text-xs text-emerald-600 mt-1">Paid + Unpaid</p>
              </div>
              <div className="bg-blue-50 p-3 sm:p-4 rounded-lg border border-blue-200">
                <p className="text-xs sm:text-sm text-blue-700 font-medium">Paid</p>
                <p className="text-xl sm:text-2xl font-bold text-blue-900">
                  {formatCurrency(filteredUsers.reduce((sum, u) => sum + u.stats.totalContributed, 0))}
                </p>
                <p className="text-xs text-blue-600 mt-1">Approved donations</p>
              </div>
              <div className="bg-amber-50 p-3 sm:p-4 rounded-lg border border-amber-200">
                <p className="text-xs sm:text-sm text-amber-700 font-medium">Unpaid</p>
                <p className="text-xl sm:text-2xl font-bold text-amber-900">
                  {formatCurrency(filteredUsers.reduce((sum, u) => sum + u.stats.pendingAmount, 0))}
                </p>
                <p className="text-xs text-amber-600 mt-1">Pending donations</p>
              </div>
            </div>
          </CardContent>
        </Card>
